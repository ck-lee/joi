...
internals.checkItems = function (items, wasArray, state, options) {
    ...
    for (var v = 0, vl = items.length, w = 0; v < vl; ++v, ++w) {
        errored = false;
        var item = items[v];
        var isValid = false;
        ...
        // Ordered
        if (this._flags.ordered) {
            res = this._inner.items[w]._validate(item, localState, options);
            if (!res.errors) {
                if (this._inner.items[w]._flags.strip) {
...
internals.Array.prototype.ordered = function (enabled) {

    var obj = this.clone();
    obj._flags.ordered = enabled === undefined ? true : !!enabled;
    return obj;
};
